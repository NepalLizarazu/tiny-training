...
from utils2 import confusion_matrix, npy_to_txt
...
class Train():
    ...
    def validate(self, model = None, mode='validation', batch_size=-1, statistics = False, integer = False, save = False, quantized = False, register_min_max = False, scales_zeros = None):
        # Validate model
        if (register_min_max):
            print("Validating and saving min and max activations")
        ...
        with torch.no_grad():
            ...
            if (save):
                ...
                outputs = model(inputs, register_min_max, quantized, scales_zeros)
    ...
    def train(self, model):
        ...
        for epoch in range(0, self.training_parameters['epochs']):
            ...
            for minibatch in range(len(data)):
                ...
                inputs, labels = data[0]
                inputs = torch.Tensor(inputs[:,None,:,:]).to(self.device)
                #saved_inputs=inputs[:20]
                #saved_inputs = saved_inputs.cpu().numpy()
                #saved_labels=labels[:20]
                #if do == 0:
                #    for i in range(20):
                #        filei = open('saved_inputs'+str(i)+'.txt', "w+")
                #        filei.write(str(saved_inputs[i][0]))
                #        #savetxt('saved_inputs'+str(i)+'.csv', saved_inputs[i][0], delimiter=',')
                #    filel = open('saved_labels.txt', "w+")
                #    filel.write(str(saved_labels))
                    #savetxt('saved_labels.csv', saved_labels, delimiter=',')
                #    do = 1
                #    print("Saved files")
            ...
            #self.set_max_weight_over_time(model)
    ...
    def set_max_weight_over_time(self, model):
        i = 0
        for name, param in model.named_parameters():
            if (str(name)[-6:] == 'weight'):
                for j in range(param.shape[0]):
                    if (model.max_weights[i,j] < torch.max(param[j]).data):
                        model.max_weights[i,j] = torch.max(param[j]).data
                    if (model.min_weights[i,j] > torch.min(param[j]).data):
